                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span aria-current="page">
                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-xs sm:text-sm font-medium bg-pink-50 border-pink-500 text-pink-500 z-10 border cursor-default leading-5">{{ $page }}</span>
                                </span>
                            @else
